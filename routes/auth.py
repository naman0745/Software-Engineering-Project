...
def register():
    ...
    Returns 403 if the same IP has already registered a node (production only).
    One-node-per-IP is NOT enforced on localhost so developers can
    register multiple test nodes freely.
    """
    ...
    # Block same IP registering multiple nodes — production only.
    # Localhost is excluded so developers can register multiple test nodes.
